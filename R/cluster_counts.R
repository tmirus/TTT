...
#' @param ids data frame or matrix assigning spatial coordinates to the spots, used 
#' for clustering in combination with the UMAP embedding, optional
...
cluster_counts <- function(counts, ids = NULL, pca.fraction = 0.8, umap.metric = "cosine", ...) {
  ...
  if(!is.null(ids)){
	...
	k <- 50
	knn.norm <- FNN::get.knn(as.matrix(embed), k = k)
	knn.norm <- data.frame(
  ...
  return(list(clustering = clustering, umap.plot = umap.plot, spatial.plot = spatial.plot))
}
